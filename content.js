...
function isArabicText(text) {
  ...
  // 1. Check if first strong direction character is Arabic
  const firstStrong = cleanText.match(/[a-zA-Z]|[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/);
  if (firstStrong && ARABIC_RE.test(firstStrong[0])) {
    return true;
  }

  // 2. Fall back to threshold ratio for the whole text
  const s = cleanText.replace(/\s+/g, '');
  ...
  const walker = document.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode(n) {
        ...
        // Optimization: skip scanning if parent is already within an RTL-marked block whose style is intact
        const closestBlock = p.closest('[data-arabic-rtl]');
        if (closestBlock && closestBlock.style.direction === 'rtl') {
